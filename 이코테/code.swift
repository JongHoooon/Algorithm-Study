...
func decodeString(_ S: String) -> String { 
    ...
    for c in inputChars { 
        if c.isNumber { 
            if !chars.isEmpty { stackChar.append(chars) }
            chars = []
            num.append(c)
            continue
        } 
        ...
        if c == "[" { 
            let count: Int = Int(String(num))!
            stackCount.append(count)
            num = []
            continue
        } 

        if c == "]" {
            let count: Int = stackCount.removeLast()
            ...
            stackChar[stackChar.count-1].append(contentsOf: newChars) 
        }
    }
    var result: [Character] = []
    for chars in stackChar { 
